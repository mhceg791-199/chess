  ...
  return (
    <div className="min-h-screen flex flex-col bg-chess-offwhite dark:bg-chess-charcoal transition-colors duration-300">
      ...
      <main className="flex-1">
        {/* HERO SECTION */}
        <section className="relative h-[380px] md:h-[400px] lg:h-[500px] overflow-hidden">
          {heroSlides.map((slide, i) => (
            <div
              key={i}
              className={`absolute inset-0 transition-opacity duration-1000 ${i === heroIndex ? "opacity-100" : "opacity-0"}`}
            >
              <img src={slide.image} alt="chess" className="w-full h-full object-cover" />
              ...
              >
                ...
                <div className="flex flex-wrap gap-3">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 bronze-gradient text-chess-charcoal dark:text-white text-sm font-semibold rounded-sm hover:opacity-90 transition-opacity"
                  >
              ...
              <div className="flex gap-2 mt-10">
                {heroSlides.map((_, i) => (
                  <button
                    ...
                    className={`h-1 rounded-full transition-all duration-500 ${i === heroIndex ? "w-10 bg-white" : "w-4 bg-white/30"}`}
                  />
                ))}
              </div>
            </div>
        ...
        {/* TRUST BAR */}
        <section className="bg-white dark:bg-chess-charcoal/50 border-b border-border dark:border-white/10">
          <div className="container py-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
              {[
                { icon: <Truck className="w-5 h-5 text-chess-bronze dark:text-white" />, title: "Free Shipping" },
                { icon: <Shield className="w-5 h-5 text-chess-bronze dark:text-white" />, title: "Extended Warranty" },
                { icon: <Headphones className="w-5 h-5 text-chess-bronze dark:text-white" />, title: "Expert Support" },
                { icon: <Clock className="w-5 h-5 text-chess-bronze dark:text-white" />, title: "Fast Delivery" },
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-sm bg-chess-cream dark:bg-white/20  flex items-center justify-center shrink-0">
                    {item.icon}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-chess-charcoal dark:text-white">{item.title}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="container">
            ...
            {loading ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
                {[...Array(10)].map((_, i) => (
                  <div key={i} className="aspect-[4/3] rounded-sm bg-chess-cream dark:bg-white/5 animate-pulse" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
                {categories.map((cat, i) => (
                  <motion.div key={cat._id} {...fadeUp} transition={{ duration: 0.4, delay: i * 0.05 }}>
                    <Link href={`/category/${cat.slug}`}>
                      <div className="group relative overflow-hidden rounded-sm bg-white dark:bg-chess-charcoal border border-border/50 dark:border-white/10 card-hover">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-chess-bronze">{categoryIcons[cat.slug] || <Package className="w-4 h-4" />}</span>
        ...
        {/* FEATURED PRODUCTS */}
        <section className="py-16 md:py-20 bg-white dark:bg-chess-charcoal/30">
          <div className="container">
            <motion.div {...fadeUp} className="flex items-end justify-between mb-10">
              ...
              <Link href="/category/all" className="hidden md:flex items-center gap-1 text-sm font-medium text-chess-bronze hover:underline">
            ...
            {loading ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="bg-chess-cream dark:bg-white/5 rounded-sm animate-pulse aspect-[3/4]" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
        ...
        {/* EDITORIAL BAND (Commercial) - Stayed mostly same as it uses fixed colors */}
            ...
            <div className="container">
              <motion.div {...fadeUp} className="ml-auto max-w-lg text-right">
                ...
                <div className="flex flex-wrap gap-3 justify-end">
                  <Link href="/b2b" className="inline-flex items-center gap-2 px-6 py-3 bronze-gradient text-chess-charcoal dark:text-white text-sm font-semibold rounded-sm hover:opacity-90 transition-opacity">
                    Request Commercial Pricing <ArrowRight className="w-4 h-4" />
                  </Link>
                </div>
              </motion.div>
            </div>
          ...
          <div className="container">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {bundles.map((bundle, i) => (
                <motion.div key={bundle.id} {...fadeUp} transition={{ duration: 0.4, delay: i * 0.1 }}>
                  <Link href="/category/all">
                    <div className="group bg-white dark:bg-chess-charcoal/50 rounded-sm border border-border/50 dark:border-white/10 overflow-hidden card-hover">
                      <div className="relative aspect-[16/9] overflow-hidden">
                        <img src={bundle.image} alt={bundle.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
                      ...
                      <div className="p-5">
                        <h3 className="font-display text-lg font-semibold text-chess-charcoal dark:text-white mb-1 group-hover:text-chess-bronze transition-colors">
                          {bundle.name}
                        </h3>
                        <div className="flex items-baseline gap-3">
                          <span className="text-xl font-semibold text-chess-charcoal dark:text-white">
        ...
        {/* SOCIAL PROOF */}
        <section className="py-20 md:py-24 bg-white dark:bg-chess-charcoal/20">
          <div className="container">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {/* Testimonial Cards */}
              {[
                { quote: "CHESS transformed our 200-unit development project...", author: "Sarah Chen", role: "VP Development" },
                { quote: "The quality of CHESS products rivals brands at twice the price...", author: "Michael Torres", role: "Director of Procurement" },
                { quote: "From ordering to installation... CHESS delivers a complete experience.", author: "David Park", role: "Licensed Contractor" },
              ].map((review, i) => (
                <motion.div key={i} {...fadeUp} className="bg-white dark:bg-chess-charcoal border border-border/50 dark:border-white/10 p-6 rounded-sm">
                  <div className="flex gap-0.5 mb-4">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-chess-bronze dark:fill-white text-chess-bronze
                     dark:text-white" />)}
                  </div>
                  <p className="text-sm text-chess-graphite dark:text-white/70 leading-relaxed mb-5 italic">"{review.quote}"</p>
                  <div>
                    <p className="text-sm font-semibold text-chess-charcoal dark:text-white">{review.author}</p>
        ...
        {/* B2B CTA BAND */}
        <section className="bg-chess-cream dark:bg-chess-charcoal py-16 border-t border-b border-border dark:border-white/20">
          <div className="container">
            <motion.div {...fadeUp} className="flex flex-col lg:flex-row items-center justify-between gap-8 max-w-5xl mx-auto">
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 rounded-sm bg-chess-charcoal dark:bg-white/20 flex items-center justify-center shrink-0">
                  <Building2 className="w-7 h-7 text-white" />
                </div>
                <div>
                  <h3 className="font-display text-xl font-semibold text-chess-charcoal dark:text-white mb-1">
                    Are you a contractor, developer, or buyer?
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    Access volume pricing and dedicated account support through our B2B portal.
                  </p>
                </div>
              </div>
              <Link href="/b2b" className="inline-flex items-center gap-2 px-6 py-3 bg-chess-charcoal dark:bg-white text-white dark:text-chess-charcoal text-sm font-semibold rounded-sm hover:opacity-90 transition-colors whitespace-nowrap">
                Apply for Commercial Account <ArrowRight className="w-4 h-4" />
              </Link>
            </motion.div>
          </div>
        </section>
