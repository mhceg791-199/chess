...
export default function AdminCustomers() {
  ...
  return (
    <AdminLayout title="Customers" subtitle={`${total} registered users`}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50 dark:border-zinc-800 bg-chess-cream/30 dark:bg-zinc-800/50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal dark:text-zinc-300">Email</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal dark:text-zinc-300">Role</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal dark:text-zinc-300">Status</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal dark:text-zinc-300">Joined</th>
                ...
                <tbody className="divide-y divide-border/30 dark:divide-zinc-800/50">
                  {users.map((u) => (
                    <tr key={u._id} className="hover:bg-chess-cream/20 dark:hover:bg-zinc-800/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-chess-cream dark:bg-white flex items-center justify-center text-xs font-semibold text-chess-bronze uppercase">
                            {u.name?.charAt(0) || "?"}
                          </div>
                          <p className="text-sm font-medium text-chess-charcoal dark:text-zinc-200">{u.name}</p>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-sm transition-colors ${u.status === "blocked"
      ...
      {/* User Detail Modal */}
      {selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setSelectedUser(null)}>
          <div
            className="bg-white dark:bg-zinc-900 rounded-sm shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto border dark:border-zinc-800"
            ...
            {detailLoading ? (
              ...
              <>
                <div className="flex items-center justify-between p-5 border-b border-border/50 dark:border-zinc-800">
                  <h2 className="font-display text-lg font-semibold text-chess-charcoal dark:text-zinc-100">Customer Details</h2>
                  <button onClick={() => setSelectedUser(null)} className="p-1 hover:bg-chess-cream dark:hover:bg-zinc-800 dark:text-zinc-400 rounded-sm transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="p-5 space-y-5">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-full bg-chess-cream dark:bg-zinc-800 flex items-center justify-center text-xl font-semibold text-chess-bronze uppercase border dark:border-zinc-700">
                      {selectedUser.name?.charAt(0) || "?"}
                    </div>
                    <div>
                      <h3 className="text-base font-semibold text-chess-charcoal dark:text-zinc-100">{selectedUser.name}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`px-2 py-0.5 text-[10px] font-semibold rounded-sm capitalize
                        ${selectedUser.role === "admin" ? "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400" : "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"}`}>
                          {selectedUser.role}
                        </span>
                        <span className={`px-2 py-0.5 text-[10px] font-semibold rounded-sm capitalize
                        ${selectedUser.status === "active" ? "bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400" : "bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400"}`}>
                  ...
                  {/* Contact Info */}
                  <div className="bg-chess-cream/30 dark:bg-zinc-800/50 rounded-sm p-4 space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      <Mail className="w-4 h-4 text-chess-bronze" />
                      <span className="text-chess-charcoal dark:text-zinc-200">{selectedUser.email}</span>
                    </div>
                    {selectedUser.phone && (
                      <div className="flex items-center gap-2 text-sm">
                        <Phone className="w-4 h-4 text-chess-bronze" />
                        <span className="text-chess-charcoal dark:text-zinc-200">{selectedUser.phone}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2 text-sm">
                  ...
                  {selectedUser.orderCount !== undefined && (
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white dark:bg-zinc-900 border border-border/50 dark:border-zinc-800 rounded-sm p-3 text-center">
                        <p className="text-lg font-semibold text-chess-charcoal dark:text-zinc-100">{selectedUser.orderCount || 0}</p>
                        <p className="text-[10px] text-muted-foreground">Total Orders</p>
                      </div>
                      <div className="bg-white dark:bg-zinc-900 border border-border/50 dark:border-zinc-800 rounded-sm p-3 text-center">
                        <p className="text-lg font-semibold text-chess-charcoal dark:text-zinc-100">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <select
                      ...
                      className="flex-1 py-2.5 text-sm font-medium border border-border dark:border-zinc-800 rounded-sm focus:outline-none focus:border-chess-bronze bg-white dark:bg-zinc-900 dark:text-zinc-200 px-2"
                    >
