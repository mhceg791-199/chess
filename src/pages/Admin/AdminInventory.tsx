        ...
        {stockFilterOptions.map((opt) => (
          <button
            ...
            className={`p-4 rounded-sm border text-left transition-all
              ${stockFilter === opt.value ? "border-chess-bronze bg-chess-bronze/5" : "border-border/50 bg-white hover:border-chess-bronze/30"}`}
          >
            <p className="text-xl font-semibold text-chess-charcoal">{opt.count}</p>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <button
          ...
          className={`h-10 px-3 flex items-center gap-1.5 text-sm border rounded-sm transition-colors
            ${bulkMode ? "border-chess-bronze bg-chess-bronze/10 text-chess-bronze" : "border-border hover:bg-chess-cream"}`}
        >
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50 bg-chess-cream/30">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">Product</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">SKU</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">Category</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">Price</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">Stock</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-chess-charcoal">Threshold</th>
                ...
                <tbody className="divide-y divide-border/30">
                  {products.map((product) => {
                    ...
                    return (
                      <tr key={product._id} className={`hover:bg-chess-cream/20 transition-colors ${hasBulkChange ? "bg-chess-bronze/5" : ""}`}>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            {product.image && (
                              <img src={product.image} alt="" className="w-8 h-8 rounded-sm object-cover" />
                            )}
                            <p className="text-sm font-medium text-chess-charcoal truncate max-w-[200px]">{product.name}</p>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm text-chess-charcoal">${(product.price || 0).toLocaleString()}</td>
                        <td className="px-4 py-3">
                          {bulkMode ? (
                            <input
                              type="number"
                              ...
                              className="w-20 h-8 px-2 text-sm border border-border rounded-sm focus:outline-none focus:border-chess-bronze text-center"
                            />
                          ) : isEditing ? (
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                className="w-20 h-8 px-2 text-sm border border-chess-bronze rounded-sm focus:outline-none text-center"
                                autoFocus
                                onKeyDown={(e) => {
                              ...
                              <button
                                ...
                                className="p-1 text-muted-foreground hover:bg-chess-cream rounded-sm"
                              >
                                <XCircle className="w-4 h-4" />
                              </button>
                            </div>
                          ) : (
                            <span className={`text-sm font-semibold ${stock === 0 ? "text-red-600" : stock <= (product.lowStockThreshold || 10) ? "text-orange-600" : "text-chess-charcoal"}`}>
                              {stock}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-sm ${ss.bg} ${ss.color}`}>
                            {ss.icon} {ss.label}
                          </span>
                        </td>
                        <td className="px-4 py-3">
