 ...
 * CHESS Header — Scandinavian Warmth + Swiss Precision
 * Warm off-white background, charcoal text, bronze accents on interactive elements.
 * Persistent top nav with mega-menu, search, cart drawer, and theme toggle.
 */
import { useState, useRef, useEffect } from "react";
import { Link, useLocation } from "wouter";
import { motion, AnimatePresence } from "framer-motion";
...
export default function Header() {
  ...
  // Logo based on theme
  const logoSrc = theme === "dark" ? LOGO.white : LOGO.dark;

  return (
    <header className="sticky top-0 z-50">
      {/* Top utility bar */}
      <div className="bg-chess-charcoal  text-chess-offwhite  text-xs transition-colors">
        <div className="container flex items-center justify-between h-8">
          <div className="hidden md:flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              <Truck className="w-3 h-3 text-white" />
      ...
      {/* Main nav */}
      <div className="bg-chess-offwhite/95 dark:bg-chess-charcoal/95 backdrop-blur-md border-b border-t  border-border shadow-sm transition-colors">
        <div className="container flex items-center justify-between h-16 lg:h-[72px]">
          {/* Mobile menu toggle */}
          <button
            className="lg:hidden p-2 -ml-2"
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            ...
            />
            <div className="hidden sm:flex flex-col leading-none opacity-0">
              <span className="font-display text-lg lg:text-xl font-bold text-chess-charcoal dark:text-chess-offwhite tracking-tight">CHESS</span>
              <span className="text-[8px] lg:text-[9px] uppercase tracking-[0.15em] text-chess-bronze font-semibold mt-0.5">by Mosaic Holding Corp.</span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex flex-1 max-w-xl mx-8">
            <form onSubmit={handleSearch} className="w-full relative">
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-10 pl-4 pr-10 rounded-sm border border-border bg-white dark:bg-black text-sm
                           focus:outline-none focus:border-chess-bronze focus:ring-1 focus:ring-chess-bronze/30
                           placeholder:text-muted-foreground transition-all"
              />
              <button type="submit" className="absolute right-0 top-0 h-10 w-10 flex items-center justify-center text-muted-foreground hover:text-chess-bronze transition-colors">
          ...
          {/* Right actions */}
          <div className="flex items-center gap-1 sm:gap-2">
            {/* Theme Toggle - Chess Pieces (Rooks) */}
            <button
              onClick={toggleTheme}
              className="hidden sm:flex items-center justify-center p-2 hover:scale-110 transition-transform"
              aria-label="Toggle theme"
              title={theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}
            >
              {theme === "dark" ? (
                // White Rook for Light Mode
                <svg className="w-5 h-5" viewBox="0 0 45 45" fill="currentColor">
                  <g fill="#fffbfb" fillRule="evenodd" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <path d="M 11,14 L 34,14" strokeLinejoin="miter" />
                  </g>
                </svg>
              ) : (
                // Black Rook for Dark Mode
                <svg className="w-5 h-5" viewBox="0 0 45 45" fill="currentColor">
                  <g fill="none" fillRule="evenodd" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M 9,39 L 36,39 L 36,36 L 9,36 L 9,39 z " fill="#000000" />
                    <path d="M 12,36 L 12,32 L 33,32 L 33,36 L 12,36 z " fill="#000000" />
                    <path d="M 11,14 L 11,9 L 15,9 L 15,11 L 20,11 L 20,9 L 25,9 L 25,11 L 30,11 L 30,9 L 34,9 L 34,14" fill="#000000" />
                    <path d="M 34,14 L 31,17 L 14,17 L 11,14" fill="#000000" />
                    <path d="M 31,17 L 31,29.5 L 14,29.5 L 14,17" fill="#000000" />
                    <path d="M 31,29.5 L 32.5,32 L 12.5,32 L 14,29.5" fill="#000000" />
                    <path d="M 11,14 L 34,14" strokeLinejoin="miter" />
                  </g>
                </svg>
              )}
            ...
            <Link href={isAuthenticated ? "/account" : "/login"} className="hidden sm:flex items-center gap-1.5 px-2 py-1.5 text-sm hover:text-chess-bronze dark:hover:text-white/70 transition-colors">
              <User className="w-4 h-4" />
              <span className="hidden md:inline">{isAuthenticated ? user?.name?.split(" ")[0] || "Account" : "Sign In"}</span>
            </Link>

            <Link href="/wishlist" className="hidden relative sm:flex items-center gap-1.5 px-2 py-1.5 text-sm hover:text-chess-bronze dark:hover:text-white/70 transition-colors">
              <Heart className="w-4 h-4" />
              <span className="hidden md:inline">Wishlist</span>
              {wishlistProducts.length > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-chess-bronze dark:bg-white text-white dark:text-black text-[10px] font-semibold w-4.5 h-4.5 rounded-full flex items-center justify-center min-w-[18px] h-[18px]">
                  {wishlistProducts.length}
                </span>
              )}
            </Link>

            <Link href="/checkout" className="hidden sm:flex items-center gap-1.5 px-2 py-1.5 text-sm hover:text-chess-bronze  dark:hover:text-white/70  transition-colors">
              <CreditCard className="w-4 h-4" />
              <span className="hidden md:inline">Checkout</span>
            </Link>

            <Link href="/cart" className="flex items-center gap-1.5 px-2 py-1.5 text-sm hover:text-chess-bronze dark:hover:text-white/70 transition-colors relative">
              <ShoppingCart className="w-5 h-5" />
              {count > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-chess-bronze dark:bg-white text-white dark:text-black text-[10px] font-semibold w-4.5 h-4.5 rounded-full flex items-center justify-center min-w-[18px] h-[18px]">
                  {count}
        ...
        <nav className="hidden lg:block border-t border-border/50">
          <div className="container flex items-center h-10">
            {topCategories.map((cat) => (
              ...
              >
                <Link
                  href={`/category/${cat.slug}`}
                  className={`flex items-center gap-1 px-3 py-2 text-[13px] font-medium tracking-wide
                    hover:text-black/70 dark:hover:text-white/70    transition-colors whitespace-nowrap
                    ${megaMenuOpen === cat._id ? "text-chess-bronze" : "text-foreground"}`}
                >
                  {cat.name}
                  <ChevronDown className={`w-3 h-3 transition-transform ${megaMenuOpen === cat._id ? "rotate-180" : ""}`} />
                ...
                <AnimatePresence>
                  {megaMenuOpen === cat._id && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 4 }}
                      transition={{ duration: 0.15 }}
                      className="absolute top-full left-0 w-[480px] bg-white dark:bg-black rounded-sm shadow-xl border border-border/50 p-5 z-50"
                      onMouseEnter={() => handleMegaEnter(cat._id)}
                      onMouseLeave={handleMegaLeave}
                    >
                      <div className="flex gap-6">
                        <div className="flex-1">
                          <h3 className="font-display text-base font-semibold mb-3 text-chess-charcoal dark:text-chess-offwhite">{cat.name}</h3>
                          <p className="text-xs text-muted-foreground mb-4">{cat.description}</p>
                          <div className="space-y-1.5">
                            {(cat.subcategories || []).map((sub) => (
                              <Link
                                key={sub.slug}
                                href={`/category/${cat.slug}/${sub.slug}`}
                                className="flex items-center justify-between py-1 text-sm hover:text-black/70 dark:hover:text-white/70 transition-colors group"
                              >
                                <span>{sub.name}</span>
                                <span className="text-xs text-muted-foreground group-hover:text-chess-bronze">{(sub.productCount ?? sub.count ?? 0).toLocaleString()}</span>
                              </Link>
                          ...
                          <Link
                            href={`/category/${cat.slug}`}
                            className="inline-block mt-4 text-xs font-semibold text-chess-bronze dark:text-white hover:underline"
                          >
                            View all {cat.name} →
                          </Link>
                        </div>
            ...
            <Link
              href="/category/all"
              className="px-3 py-2 text-[13px] font-medium tracking-wide text-chess-bronze dark:text-white hover:underline whitespace-nowrap ml-auto"
            >
              All Categories
            </Link>
          </div>
      ...
      <AnimatePresence>
        {searchOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="lg:hidden absolute top-full left-0 right-0 bg-white dark:bg-black border-b border-border p-3 shadow-lg z-40"
          >
            <form onSubmit={handleSearch} className="relative">
              <input
                ref={searchRef}
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-10 pl-4 pr-10 rounded-sm border border-border dark:bg-black text-sm focus:outline-none focus:border-chess-bronze"
              />
              <button type="submit" className="absolute right-0 top-0 h-10 w-10 flex items-center justify-center">
                <Search className="w-4 h-4" />
              </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -300 }}
            transition={{ type: "spring", damping: 25, stiffness: 200 }}
            className="lg:hidden fixed inset-0 top-[calc(4rem+2rem)] bg-white dark:bg-black z-40 overflow-y-auto"
          >
            <div className="p-4 space-y-1">
              {/* Theme toggle in mobile menu */}
              <button
                onClick={toggleTheme}
                className="flex items-center justify-between w-full py-3 px-2 text-sm font-medium border-b border-border/30 hover:text-chess-bronze transition-colors"
              >
                <span>Theme</span>
                <span className="flex items-center gap-2">
                  {theme === "dark" ? (
                    ...
                    <>
                      ...
                      <svg className="w-5 h-5" viewBox="0 0 45 45" fill="currentColor">
                        <g fill="none" fillRule="evenodd" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M 22,9 C 19.79,9 18,10.79 18,13 C 18,13.89 18.29,14.71 18.78,15.38 C 16.83,16.5 15.5,18.59 15.5,21 C 15.5,23.03 16.44,24.84 17.91,26.03 C 14.91,27.09 10.5,31.58 10.5,39.5 L 33.5,39.5 C 33.5,31.58 29.09,27.09 26.09,26.03 C 27.56,24.84 28.5,23.03 28.5,21 C 28.5,18.59 27.17,16.5 25.22,15.38 C 25.71,14.71 26,13.89 26,13 C 26,10.79 24.21,9 22,9 z " fill="#000000" stroke="currentColor" />
                          <path d="M 9.5,25.5 A 0.5,0.5 0 1 1 8.5,25.5 A 0.5,0.5 0 1 1 9.5,25.5 z" fill="#fffbfb" stroke="#fffbfb" />
                          <path d="M 15,15.5 A 0.5,1.5 0 1 1 14,15.5 A 0.5,1.5 0 1 1 15,15.5 z" fill="#fffbfb" stroke="#fffbfb" transform="matrix(0.866,0.5,-0.5,0.866,9.693,-5.173)" />
                        </g>
                      </svg>
                      Dark
                    </>
